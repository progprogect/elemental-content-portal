...
export async function getDebugFrames(req: Request, res: Response) {
  try {
    ...
    // Get debug frames from sceneProject.extra.debugFrames
    const sceneProject = scene.sceneProject as any;
    ...
    logger.error({ error, generationId: req.params.generationId, sceneId: req.params.sceneId }, 'Failed to get debug frames');
    res.status(500).json({ error: 'Failed to get debug frames', message: error.message });
  }
}
